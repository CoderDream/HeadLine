    ...
    @IBAction func concernButtonClicked(_ sender: AnimatableButton) {
        ...
        if sender.isSelected { // 已经关注，点击则取消关注
            ...
            NetworkTool.loadRelationFollow(user_id: userCard!.user.info.user_id, completionHandler: { (_) in
                ...
                self.concernButton.borderColor = userDetail!.is_following ? UIColor.grayColor232() : UIColor.globalRedColor()
                self.concernButton.borderWidth = userDetail!.is_following ? 1 : 0
            })
        }
    }
